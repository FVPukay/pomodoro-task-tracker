...
describe('Tasks Component', () => {
  ...
  describe('Add Task Form', () => {
    it('should not have priority input in add task form', () => {
      render(<Tasks />);

      // The add task form should only have the text input and Add button
      // No priority input should exist in the form
      ...
      // Priority input should NOT be in the add form (it's now on task items)
      // Check that no priority input exists before any tasks are created
      const priorityInputs = screen.queryAllByLabelText('Task priority');
      expect(priorityInputs).toHaveLength(0);
    });

    it('should create tasks with default priority of 4', async () => {
      const user = userEvent.setup();
      render(<Tasks />);
      ...
      // Add task
      await user.type(taskTitleInput, 'New Default Task{Enter}');

      // Task should be created
      await screen.findByText('New Default Task');

      // Check localStorage to verify priority was set to default 4
      await waitFor(() => {
        const saved = localStorage.getItem('pomodoro-tasks');
        expect(saved).not.toBeNull();
        const parsed = JSON.parse(saved!);
        expect(parsed.tasks[0].priority).toBe(4);
      });
    });
  });
